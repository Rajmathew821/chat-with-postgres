...
class RAGSafetyStockSystem:
    def __init__(self, db_config: DatabaseConfig, model_name: str = "meta-llama/Llama-2-7b"):
        ...
        self.nl_to_sql_prompt = PromptTemplate(
            input_variables=["table_name","schema", "question","columns"],
            template="""
    ...
    def find_schema(self, table_name: str) -> str:
        try:
            with self.db_connection.get_cursor() as cursor:
                ...
                cols = [col['column_name'] for col in results]
                return converted_sql,cols
    ...
    def generate_sql_query(self, question: str,table_schema: str,table_name: str,columns: List) -> str:
        try:
    ...
    def process_query(self, question: str) -> Dict[str, Any]:
        try:
            ...
            table_name = table_name.lower().strip().strip('`')
            logger.info(f"Found Table Name: {table_name}")

            table_schema,columns = self.find_schema(table_name)
            logger.info(f"Found Table Schema: {table_schema}")

            sql_query = self.generate_sql_query(question,table_schema,table_name,columns)
